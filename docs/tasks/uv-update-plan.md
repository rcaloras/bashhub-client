...
- Latest updates use `uv tool install --python 3.13 --reinstall bashhub`.
...
- Install or reinstall the unpinned package with
  `uv tool install --python 3.13 --reinstall bashhub` for the latest version.
  This clears any exact version constraint left by an earlier rollback.
...
```bash
uv tool install --python 3.13 --reinstall bashhub
